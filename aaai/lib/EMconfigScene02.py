...
src_dir = '/data/GeometricErrors/aaai_data/scene02'
...
weightBuffer = wrap( 15 , Annotations)
# Buffered used when rating the optimal frame quality 
precisionBuffer = wrap( 15 , Annotations)
# -----------------------------------------
...
# Nodata is -1e-4, but doubt we'll need it

# Ground Truth Label Path
gt_labels = wrap_fp(src_dir + '/refined_lines_smaller_area.shp', sourceData)
# Imperfect Label Path
imp_labels = wrap_fp(src_dir + '/imperfect_lines_smaller_dataset.shp', sourceData)

# Raster for testing area
test_raster = wrap_fp(src_dir + '/test_raster.tif', sourceData)
# Raster for training area
train_raster = wrap_fp(src_dir + '/train_raster.tif', sourceData)

# Pre-Trained Model's output pmap (Needed for generating candidate windows when weighting segments)
initial_pmap = wrap_fp(src_dir + '/train_raster.tif', sourceData)
