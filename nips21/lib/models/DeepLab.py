...
from keras.layers import Add, AveragePooling2D,Reshape
...
""" 
DeepLab Model with input shape (224, 224) for Geometric Errors Project. 
...
def Deeplabv3(input_tensor=None, infer = False,
              input_shape=(224, 224, 7), classes=1, backbone='xception',
              OS=16, alpha=1.):
    ...
    batches_input = img_input #Lambda(lambda x: x/127.5 - 1)(img_input)
...
def Deeplabv4(input_shape=(224, 224, 7), OS=16, alpha=1.):
    
    """ 
    Modified Deeplab model to remove weight persistence.  
    ...
    if K.backend() != 'tensorflow':
        print("Keras Environment error, use tensorflow backend. Current: {}".format(K.backend()))
