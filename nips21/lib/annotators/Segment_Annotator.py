...
seg_kwargs_key.update(("Annotator - {}".format(key), item) for key, item in anno_kwargs_key.items())

class Segment_Annotator(Annotator):
    """ 
    ...
    def __init__(self, **kwargs):
        ...
        default_attr = dict(
            ...
            epsilon = 0, # Chance to select a random coordinate from top points neighbors
            k = 0,    # Range on either side of top point to select random candidate
            kwargs_key = seg_kwargs_key,
            min_p = 1e-06
        )
    ...
    def select_segment(self, start_group, end_group, class_map):
        ...
        candidate_lines = [shp.LineString([p_a, p_b]) for p_a, p_b in zip(start_group.geoms, end_group.geoms)]
        ...
        # DEFAULT TO OUTERMOST
        top_weight = 0.0
        top_idx = None
        default_idx = len(candidate_lines)-1
        ...
        for idx, candidate_line in enumerate(candidate_lines):
            ...
            mask = rfeat.geometry_mask([candidate_line.buffer(self.weight_buffer)], out_shape=group_data.shape, transform=rwin.transform(group_window, class_map.transform), all_touched=True)
        ...
        return candidate_lines[self._roll_random_candidate(source_idx=top_idx, candidate_group=candidate_lines)]
    ...
    def _new_annotation(self, line, class_map=None):
        ...
        for idx, group in enumerate(candidate_groups):
            if (idx == 0):
                ...
                segment = self.select_segment(start_group=prev_group, end_group=group, class_map=class_map)
                optimal_segments.append(segment)
                prev_group = group

        return shp.MultiLineString(optimal_segments)
    ...
    def update_gdf(self, gdf, class_map, weight_buffer=2, label_offsets=False, clip=True, out_path=None):
        ...
        gdf = gpd.clip(gdf, self.crop_window, keep_geom_type=True)
        ...
        annotation_frame = gpd.clip(annotation_frame, mask=self.crop_window, keep_geom_type=True)
